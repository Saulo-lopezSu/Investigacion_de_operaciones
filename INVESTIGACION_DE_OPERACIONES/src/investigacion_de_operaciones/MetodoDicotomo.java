//METODO DE DICOTOMO CORRESPONDIENTE A LA UNIDAD 3
package investigacion_de_operaciones;

import java.util.ArrayList;
import java.util.Scanner;

public class MetodoDicotomo {
    // public void ejecusionDicotomo() {//metodo main 
     public static double f(double x) {
        return Math.pow(x, 4) - 14 * Math.pow(x, 3) + 60 * Math.pow(x, 2) - 70 * x;
    }

    public void ejecusionDicotomo() {
        ...
        // Iniciar el algoritmo de dicotomía
        int iteraciones = 0;
        ArrayList<Double> valores_f = new ArrayList<>();
        while ((b - a) > precision) {
            iteraciones++;
            double x1 = (a + b - precision) / 2;
            double x2 = (a + b + precision) / 2;
